...
if today_decisions.empty:
    ...
    version_sel = st.radio(
        ...
        help=(
            ...
            "頑健性が確認できず非推奨)、v3.0=グレードB/C×52週高値接近×割安性上位50%"
            "(1〜2年保有想定、2022〜2024年の全年で頑健な優位性を確認済み)、"
            "v4.0=v3.0の条件にキャッシュニュートラルPER下位40%を追加"
            "(清原達郎氏のPER×(1-ネットキャッシュ比率)、ネットキャッシュ比率=(流動資産"
            "+投資有価証券×0.7-負債)/時価総額、2022〜2024年の全年で24ヶ月保有リターンが"
            "v3.0をさらに上回ることを確認済み、ただし銀行・保険業はデータ取得不可のため"
            "対象外、検証中)"
        ),
    )
...
filters_active = bool(
    search or sector_sel != "すべて" or set(grade_sel) != set(grades) or score_min > 0
)
...
# 検索語だけで銘柄が一意に絞り込める場合は、選択操作を挟まずその銘柄の詳細へ直接ジャンプする
st.session_state.setdefault("last_auto_search", None)
if search:
    ...
    if (
        ...
        st.session_state["jump_pending"] = True
elif st.session_state["last_auto_search"] is not None:
    st.session_state["last_auto_search"] = None
...
# 買い/売り一覧のクリック、検索での自動選択、またはサイドバー絞り込みで選択中の銘柄が
# 候補から消えても選択状態を保てるよう、現在値がoptionsに無ければ先頭に補って残す
...
if selected_label:
    ...
    def _render_glance_row(items: list[tuple[str, str, str]]) -> None:
        """1行目・2行目で列位置が揃うよう、常にGLANCE_ROW_COLS列で描画する(余りは空欄)"""
        cols = st.columns(GLANCE_ROW_COLS)
